...
class TreeCleaner(object):
    ...
    def __init__(self, tree, save_reports=False, nesting_strictness='loose', status_cb=None, rtl=False):
        ...
        self.protected_css_classes = ['fa', 'protected']
    ...
    def removeListOnlyParagraphs(self, node):
        ...
        if node.__class__ == Paragraph:
            ...
            if list_only_children and node.parent and not any(val in self.protected_css_classes for val in css_class):
    ...
    def _fixNesting(self, node):
        ...
        new_tree = [part for part in [top_tree, middle_tree, bottom_tree] if part is not None]
    ...
    def fixReferenceNodes(self, node):
        ...
        for ref_node in ref_nodes:
            ...
            if ref_name and ref_node.children and not ref_name in name2children:
                name2children[ref_name] = ref_node.children

        ref_defined = {}
        for ref_node in ref_nodes:
            ref_name = ref_node.attributes.get('name')
            if not ref_name or not ref_name in name2children:
                continue
